...
const ChatCard = ({isUnseen, convoPeople }: { isUnseen: boolean, convoPeople: ChattedUserType }) => {
    return (
        <Link to={`/dash/chat/${convoPeople._id}`} className={` relative flex items-end gap-2 bg-gray-100 p-3 rounded-lg cursor-pointer`}>
            ...
            {isUnseen && <span className="absolute h-6 w-6 rounded-full bg-green-800">d</span>}
        </Link>
    );
};
